...
class Case2Frame():
	def __init__(self, master):
		...
		self.working_var = StringVar()
		self.working_spinbox = Spinbox(self.content, from_=0, to=60, textvariable=self.working_var, wrap=True)
		self.working_var.set('60')
		...
		self.tw1_entry = ttk.Entry(self.wbgt1_content, textvariable=self.tw1_var, width=10)
		...
		self.tg1_entry = ttk.Entry(self.wbgt1_content, textvariable=self.tg1_var, width=10)
		...
		self.td1_entry = ttk.Entry(self.wbgt1_content, textvariable=self.td1_var, width=10)
		...
		# select rest section
		self.rest_label = ttk.Label(self.content, text='Selecione o tipo de descanso', anchor='center', padding=(0, 40, 0, 0))
		...
		self.resting_var = StringVar()
		self.resting_spinbox = Spinbox(self.content, from_=0, to=60, textvariable=self.resting_var, wrap=True)
	...
	def adjust_resting(self, event):
		try:
			...
			return
		if working_period > 60:
			self.working_var.set('60')
			self.resting_var.set('0')
			return
		elif working_period < 0:
			self.working_var.set('0')
			self.resting_var.set('60')
			return
		new_working_var = int(working_period)
	...
	def adjust_working(self, event):
		try:
			...
			return
		if resting_period > 60:
			self.working_var.set('0')
			self.resting_var.set('60')
			return
		elif resting_period < 0:
			self.working_var.set('60')
			self.resting_var.set('0')
			return
		new_resting_var = int(resting_period)
		self.resting_var.set(str(new_resting_var))
